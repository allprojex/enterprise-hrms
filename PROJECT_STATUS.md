...
- **W6 — Frontend Module Gating — Not started (next).**
- W7–W21 — Not started.
...
1. No Module Management *enforcement* — the registry (W3), per-org enablement (W4), and the backend gate mechanism (W5) now exist, but no route composes the gate yet (nothing to gate) and there's no frontend gating yet (W6). Everything still ships unconditionally on for every org.
