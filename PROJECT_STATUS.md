...
- Module Management — In Progress. Module Registry exists (W3, `GET /modules`, 8 HR-operations modules seeded as `status: hidden`). No per-org enable/disable yet (W4), no gating anywhere yet (W5/W6). Everything shipped so far is still unconditionally on for every org.
...
- **W4 — Per-Organization Module Enablement — Not started (next).**
- W5–W21 — Not started.
...
1. No Module Management *enforcement* — the registry now exists (W3), but no organization can enable/disable a module yet (W4) and nothing gates on it yet (W5/W6). Everything still ships unconditionally on for every org.
