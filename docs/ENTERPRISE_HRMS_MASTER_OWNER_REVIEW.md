...
| WS-22 | Fleet Agent & Executor Credentials | **Reserved** | Identifier reserved by the owner (2026-09-06) for per-installation agent/executor credential issuance, rotation and revocation — the secrets subsystem WS-17 deferred. Scope not yet frozen. | WS-17 | — (**reserved, not specified, not implemented**) |
| WS-25 | Enterprise UI/UX Modernization | **P1** | Premium, professional enterprise design system applied in phases: **WS-25A Design Foundation** (tokens, self-hosted typography, light theme with dark-ready tokens, tenant-brand-on-top-of-system architecture with readability clamp, button/form/card/badge/table/dialog/toast primitives, motion + reduced-motion, loading/empty/error states, responsive + accessibility foundations, dev-only showcase) → B authentication & branding surfaces → C shell/navigation → D core HR surfaces → E Super Admin surfaces → F polish/dark toggle. Frontend only; no schema or API change. Blueprint: `docs/UI_MODERNIZATION_BLUEPRINT.md`. | none (frontend); WS-18 host/tenant protections preserved | **WS-25A COMPLETE** (2026-09-06, `docs/WS25_DESIGN_FOUNDATION.md`); B–F not started. Not deployed. |
| WS-26 | Tenant Form, Workflow & Signature Engine | **P1** | Tenant-scoped official forms as immutable versioned templates with per-version workflow stages, append-only submissions and chronology, structured PDF rendering (blank / draft / submitted / returned / rejected / approved / immutable final with SHA-256), reusable signature engine, and the four Worldwide Word Ministries forms (Leave Application, Personal Information, Staff Evaluation, Probationary Assessment) as organization-3 data with a docx-derived fidelity harness. Phases: **26A foundation** → 26B signature & finalized-document engine → 26C WWM Leave + PIF integration → 26D WWM Evaluation + Probation integration → 26E fidelity, history & Production readiness. Architecture: `docs/WS26_FORM_WORKFLOW_SIGNATURE_ARCHITECTURE.md`. | WS-5 (generated documents), WS-8 (custom fields for tenant-specific PIF data), WS-13 (stage resolver vocabulary), Leave, Performance, WS-18 protections preserved | **WS-26A COMPLETE** (2026-09-06, `docs/WS26A_FOUNDATION.md`, migration 0076); B–E not started. Not deployed; Production not migrated or seeded. |
