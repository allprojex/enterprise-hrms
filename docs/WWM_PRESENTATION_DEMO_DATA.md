...
## Personnel Files & Documents Demo Data

Both systems are foundation features (not module-gated) that already existed in the codebase and were simply unused for WWM until now — nothing new was built.

- **Records Location:** "HR Records Room" (id 10) — the physical storage location personnel files are tracked against.
- **Personnel Files (PIF registry):** all 6 employees now have a generated PIF number (`PIF-011` through `PIF-016`, org-scoped sequence continues from earlier historical allocations). Each file's custody state is tracked (`in_registry`, `checked_out`, etc.) — no PDF/paper document is generated by this; it is a records-registry and location/custody tracker only. This directly answers the earlier, disclosed finding that no PIF *document* generator exists in this codebase — the PIF *number/registry* system does exist and is now populated for WWM.
- **Custody lifecycle demonstrated:** Kwame Owusu's personnel file (PIF-011) was checked out ("Payroll Audit Review" destination) then returned to HR Records Room — proving the full checkout/return cycle works, not just static allocation.
- **Employee Documents (real, valid PDF files, not fabricated data pretending to be something else):** three minimal, clearly-labelled placeholder PDFs, each stating "WWM Presentation Readiness -- Demo Document" and identifying itself by name/category in its own text, uploaded through the real `POST /organizations/3/employees/:id/documents` endpoint (signature-validated, stored via the same organization-scoped file storage as everything else in this codebase):
  - Kwame Owusu — `IDENTIFICATION` category, `kwame-id-copy.pdf`
  - Grace Mensah — `CONTRACT` category, `grace-employment-contract.pdf`
  - Ama Boateng — `CERTIFICATE` category, `ama-qualification-certificate.pdf`
