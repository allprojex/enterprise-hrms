...
A resolved tenant hostname is checked for consistency at exactly four points, all reusing one function (`hostnameOrganizationMismatch`, `lib/organizationDomains.ts`):
...
3. **`POST /auth/login`** — after password verification succeeds and before a session is issued: if a tenant hostname resolved and the account has no active membership there, the login itself is denied (`403`, "This account does not have access to this organization") — even with fully correct credentials.
4. **`POST /auth/switch-organization`** — switching into an organization that mismatches the resolved hostname is denied the same way. Per the brief, a hostname change never triggers an automatic switch either way — nothing in this workstream reads the resolved hostname to *pick* an organization on its own; it only ever narrows what a caller-initiated action is allowed to target.

**`super_admin` is exempt** from all four checks — the same platform-wide bypass `isSuperAdmin()`/`authorizeOrganizationAction()` already grants everywhere else in this codebase. A platform super-admin can log in and act from any hostname; an ordinary tenant user cannot cross a hostname boundary even with valid credentials for the account itself.
...
**A pre-existing, platform-wide condition surfaced by Supabase's own security advisor, not introduced or fixed by this workstream:** Row Level Security is disabled on every table in the public schema, `organization_domains` included — consistent with every other table, since this application authorizes exclusively at the Express/API layer (session-token auth, not Supabase Auth/PostgREST), not via Postgres RLS. Enabling RLS without matching policies would break every existing table's access, so it was left as-is and is surfaced here per the advisor's own instruction, for your review — not something this workstream should have silently changed.
