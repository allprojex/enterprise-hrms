...
export const ListLeaveRequestsResponseItem = zod.object({
  ...
  "status": zod.enum(['pending', 'pending_hr', 'approved', 'rejected', 'cancelled']).describe('\"pending\" = awaiting the employee\'s current Department Head; \"pending_hr\" = Department Head approved, awaiting HR\'s final decision. See `workflowStage` for the 6 owner-facing labels (also distinguishes which stage a \"rejected\" request was rejected at).'),
  "workflowStage": zod.enum(['awaiting_department_head', 'awaiting_hr', 'rejected_by_department_head', 'approved', 'rejected_by_hr', 'cancelled']).optional().describe('Derived, never stored — the exact stage label HR\/ESS should render.'),
  "reason": zod.string().nullish(),
  "attachmentDocumentId": zod.number().nullish(),
  "cancelledAt": zod.coerce.date().nullish(),
  "cancelledBy": zod.number().nullish(),
  "departmentHeadApprovedBy": zod.number().nullish().describe('The user id of the Department Head who approved this request\'s first stage. Never overwritten by a later HR action or a subsequent Department Head replacement.'),
  "departmentHeadApprovedAt": zod.coerce.date().nullish(),
  "departmentHeadRejectedBy": zod.number().nullish(),
  "departmentHeadRejectedAt": zod.coerce.date().nullish(),
  "departmentHeadRejectionReason": zod.string().nullish(),
  "approvedBy": zod.number().nullish().describe('The user id who gave FINAL (HR-stage) approval.'),
  "approvedAt": zod.coerce.date().nullish(),
  "rejectedBy": zod.number().nullish().describe('The user id who gave the FINAL (HR-stage) rejection. Null when the request was instead rejected at the Department Head stage — see departmentHeadRejectedBy.'),
  "rejectedAt": zod.coerce.date().nullish(),
  "rejectionReason": zod.string().nullish().describe('The HR-stage rejection reason. See departmentHeadRejectionReason for a Department-Head-stage rejection.'),
  "createdBy": zod.number().nullish(),
  "createdAt": zod.coerce.date(),
  "updatedAt": zod.coerce.date()
})
...
export const RejectLeaveRequestBody = zod.object({
  "reason": zod.string().min(1)
}).describe('Rejecting a leave request (at either the Department Head or HR stage) always requires a non-empty reason — Leave Approval Workflow Reconciliation binding rules #5\/#6.')
 ...
 */
export const ListOfficeInventoryReceiptsParams = zod.object({
  "organizationId": zod.coerce.number()
})

export const ListOfficeInventoryReceiptsResponseItem = zod.object({
  "referenceNumber": zod.string(),
  "storeId": zod.number(),
  "occurredAt": zod.coerce.date(),
  "lineCount": zod.number()
})
export const ListOfficeInventoryReceiptsResponse = zod.array(ListOfficeInventoryReceiptsResponseItem)
 ...
 */
export const CreateOfficeInventoryReceiptParams = zod.object({
  "organizationId": zod.coerce.number()
})
...
export const CreateOfficeInventoryReceiptBody = zod.object({
  "storeId": zod.number(),
  "lines": zod.array(zod.object({
  "itemId": zod.number(),
  "quantity": zod.string().describe('Positive numeric(12,2)-shaped string.'),
  "unitCost": zod.string().optional()
})).min(1),
  "source": zod.string().optional(),
  "deliveryReference": zod.string().optional(),
  "notes": zod.string().optional(),
  "idempotencyKey": zod.string().optional().describe('Optional client-generated key protecting against duplicate submission (e.g. a double click or network retry). Same key + same operation returns the original receipt rather than creating a second one.')
})

export const CreateOfficeInventoryReceiptResponse = zod.object({
  "referenceNumber": zod.string(),
  "organizationId": zod.number(),
  "storeId": zod.number(),
  "occurredAt": zod.coerce.date(),
  ...
  "replay": zod.boolean().describe('True when this response is the pre-existing result of an earlier identical idempotencyKey submission, not a newly-created receipt.')
}).describe('Not a separate DB entity — the set of `received` ledger rows sharing one `referenceNumber`, assembled on read.')


/**
 * @summary Get one receipt by its reference number
 */
export const GetOfficeInventoryReceiptParams = zod.object({
  "organizationId": zod.coerce.number(),
  "referenceNumber": zod.coerce.string()
})

export const GetOfficeInventoryReceiptResponse = zod.object({
  "referenceNumber": zod.string(),
  "organizationId": zod.number(),
  "storeId": zod.number(),
  "occurredAt": zod.coerce.date(),
 ...
 */
export const GetOfficeInventoryStockBalanceParams = zod.object({
  "organizationId": zod.coerce.number()
})

export const GetOfficeInventoryStockBalanceQueryParams = zod.object({
  "itemId": zod.coerce.number(),
  "storeId": zod.coerce.number().optional().describe('If supplied, `total` is that one store\'s balance and `byStore` is empty. If omitted, `total` is the organization-wide total and `byStore` lists every store the item has moved through.')
})

export const GetOfficeInventoryStockBalanceResponse = zod.object({
  "itemId": zod.number(),
 ...
 */
export const ListOfficeInventoryStockMovementsParams = zod.object({
  "organizationId": zod.coerce.number()
})

export const ListOfficeInventoryStockMovementsQueryParams = zod.object({
  "itemId": zod.coerce.number().optional(),
  "storeId": zod.coerce.number().optional()
})

export const ListOfficeInventoryStockMovementsResponseItem = zod.object({
  "id": zod.number(),
  "organizationId": zod.number(),
  "itemId": zod.number(),
  "movementType": zod.enum(['received', 'issued', 'returned', 'transferred_out', 'transferred_in', 'adjustment_in', 'adjustment_out', 'written_off', 'missing', 'recovered', 'asset_handoff']),
 ...
 */
export const CreateOfficeInventoryReturnParams = zod.object({
  "organizationId": zod.coerce.number()
})

export const CreateOfficeInventoryReturnBody = zod.object({
  "itemId": zod.number(),
  "holderType": zod.enum(['employee', 'department']),
  "holderId": zod.number(),
...
export const CreateOfficeInventoryReturnResponse = zod.object({
  "storeMovement": zod.object({
  "id": zod.number(),
  "organizationId": zod.number(),
  "itemId": zod.number(),
 ...
 */
export const CreateOfficeInventoryHandoverParams = zod.object({
  "organizationId": zod.coerce.number()
})

export const CreateOfficeInventoryHandoverBody = zod.object({
  "itemId": zod.number(),
 ...
 */
export const CreateOfficeInventoryTransferParams = zod.object({
  "organizationId": zod.coerce.number()
})

export const CreateOfficeInventoryTransferBody = zod.object({
  "itemId": zod.number(),
